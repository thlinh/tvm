...
def differentiate(output, inputs=None, head=None, manual=None, fdiff=None):
    ...
    if manual is not None:
        ...
        def _modified_fdiff(out, inp, hd, manual=manual, old_fdiff=fdiff, used_items=used_items):
            if (out, inp) in manual:
                used_items.add((out, inp))
                return manual[(out, inp)](out, inp, hd)
            elif (out, None) in manual:
                used_items.add((out, None))
                return manual[(out, None)](out, inp, hd)
            elif (None, inp) in manual:
                used_items.add((None, inp))
                return manual[(None, inp)](out, inp, hd)
            else:
                return old_fdiff(out, inp, hd)
