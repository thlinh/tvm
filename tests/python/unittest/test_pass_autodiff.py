...
def test_stride_dilation():
    ...
    test_grad(Y, [X, W], perf=(1608, 9220, 604))
    Y = topi.nn.conv2d(X, W, 2, 0, 1)
    test_grad(Y, [X, W], perf=(1566, 14710, 654))
    Y = topi.nn.conv2d(X, W, 3, 0, 1)
    test_grad(Y, [X, W], perf=(1532, 11284, 636))
    Y = topi.nn.conv2d(X, W, 1, 0, 2)
    test_grad(Y, [X, W], perf=(1608, 9220, 604))
    Y = topi.nn.conv2d(X, W, 2, 0, 2)
    test_grad(Y, [X, W], perf=(1170, 10718, 454))
    Y = topi.nn.conv2d(X, W, 3, 0, 2)
    test_grad(Y, [X, W], perf=(1136, 8092, 268))
    ...
    Y = topi.nn.conv2d(X, W, 1, 0, 1)
    test_grad(Y, [X, W], perf=(4202, 42580, 1106))
    Y = topi.nn.conv2d(X, W, 2, 0, 1)
    test_grad(Y, [X, W], perf=(1850, 18548, 650))
    Y = topi.nn.conv2d(X, W, 3, 0, 1)
    test_grad(Y, [X, W], perf=(1576, 18136, 632))
    Y = topi.nn.conv2d(X, W, 1, 0, 2)
    test_grad(Y, [X, W], perf=(3860, 56672, 1000))
    Y = topi.nn.conv2d(X, W, 2, 0, 2)
    test_grad(Y, [X, W], perf=(4178, 147040, 1832))
    Y = topi.nn.conv2d(X, W, 3, 0, 2)
    test_grad(Y, [X, W], perf=(3290, 173288, 1112))
    Y = topi.nn.conv2d(X, W, 1, 0, 3)
    test_grad(Y, [X, W], perf=(3578, 53116, 930))
    Y = topi.nn.conv2d(X, W, 2, 0, 3)
    test_grad(Y, [X, W], perf=(4082, 128716, 1898))
    Y = topi.nn.conv2d(X, W, 3, 0, 3)
    test_grad(Y, [X, W], perf=(4034, 126804, 1890))

    W = tvm.placeholder((2, 2, 3, 3), name='W')

    Y = topi.nn.conv2d(X, W, 1, 0, 1)
    test_grad(Y, [X, W], perf=(7780, 81668, 1676))
    Y = topi.nn.conv2d(X, W, 2, 0, 1)
    test_grad(Y, [X, W], perf=(4658, 109612, 1832))
    Y = topi.nn.conv2d(X, W, 3, 0, 1)
    test_grad(Y, [X, W], perf=(1880, 19792, 632))
    Y = topi.nn.conv2d(X, W, 1, 0, 2)
    test_grad(Y, [X, W], perf=(6436, 106492, 1240))
    Y = topi.nn.conv2d(X, W, 2, 0, 2)
    test_grad(Y, [X, W], perf=(8266, 163862, 4018))
    Y = topi.nn.conv2d(X, W, 3, 0, 2)
    test_grad(Y, [X, W], perf=(3952, 360556, 1880))
    Y = topi.nn.conv2d(X, W, 1, 0, 3)
    test_grad(Y, [X, W], perf=(3096, 134564, 520))
    Y = topi.nn.conv2d(X, W, 2, 0, 3)
    test_grad(Y, [X, W], perf=(2786, 53092, 1880))
    Y = topi.nn.conv2d(X, W, 3, 0, 3)
    test_grad(Y, [X, W], perf=(2424, 44444, 280))

    Y = topi.nn.pool(X, [1, 1], [1, 1], [0, 0, 0, 0], 'max')
    test_grad(Y, [X], perf=(400, 1200, 200))
    ...
    test_grad(Y, [X], perf=(4442, 140252, 1842))
    ...
    test_grad(Y, [X], perf=(18488, 654528, 3888))
